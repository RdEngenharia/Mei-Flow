...
export default function CnpjOnboarding({ onSave, onSkipWithDemo, userEmail }: CnpjOnboardingProps) {
  ...
  const handleLookup = async (e: React.FormEvent) => {
    ...
    if (cleanCnpj.length !== 14) {
      setErrorMsg("O CNPJ deve conter exatamente 14 dÃ­gitos.");
      return;
    }
    setSearching(true);
    setErrorMsg("");
    ...
    try {
      ...
      const response = await fetch(`${apiBase}/api/cnpj/lookup?cnpj=${cleanCnpj}`);
      ...
      if (ok) {
        ...
        setErrorMsg("CNPJ nÃ£o localizado automaticamente nas bases federais. Digite os dados abaixo manualmente:");
        setMeiName("");
        setTelefone("");
        setStep(2);
      }
      ...
      setErrorMsg("CNPJ nÃ£o encontrado nas bases pÃºblicas ou serviÃ§o temporariamente indisponÃ­vel. Preencha manualmente para prosseguir.");
      setMeiName("");
      setTelefone("");
      setStep(2);
    } finally {
  ...
  const handleFinalSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!meiName.trim()) {
      setErrorMsg("Insira a RazÃ£o Social ou Nome do Emissor.");
      return;
    }
    setSaving(true);
    try {
    ...
    <div className="w-full max-w-lg mx-auto bg-white rounded-3xl border border-slate-100 shadow-2xl overflow-hidden p-6 md:p-8 space-y-6 animate-fade-in my-10 relative">
      ...
      <div className="space-y-2 text-center relative z-10">
        <div className="inline-flex items-center gap-1 bg-blue-50 text-blue-600 font-bold text-[10px] uppercase tracking-widest px-3 py-1.5 rounded-full shadow-xs">
          ...
          <span>ConfiguraÃ§Ã£o Cadastral Inicial</span>
        </div>
        <h2 className="text-2xl md:text-3xl font-display font-light text-slate-900 tracking-tight">
          Pronto para decolar, <span className="font-semibold text-blue-600 truncate max-w-[200px] inline-block align-bottom">{userEmail.split("@")[0]}</span>? ðŸš€
        </h2>
        <p className="text-xs text-slate-400 max-w-md mx-auto leading-relaxed">
          Como criamos uma conta segura no Mei Flow, precisamos apenas do CNPJ do seu MEI para carregar seus dados e configurar todas as emissÃµes automaticamente.
        </p>
      </div>
      ...
      <div className="flex items-center justify-center gap-6 py-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-[11px] font-bold ${step === 2 ? "text-slate-800" : "text-slate-400"}`}>Confirmar InformaÃ§Ãµes</span>
        </div>
      </div>
        ...
        <form onSubmit={handleLookup} className="space-y-4 pt-2 relative z-10">
          ...
          <div className="flex flex-col gap-3 pt-2">
            ...
            >
              ...
              <span>Experimentar aplicativo com MEI de DemonstraÃ§Ã£o</span>
            </button>
          </div>
        </form>
      ) : (
        <form onSubmit={handleFinalSave} className="space-y-4 pt-2 relative z-10">
          ...
          <div className="p-4 bg-emerald-50/80 border border-emerald-100 rounded-2xl text-emerald-800 text-xs font-medium space-y-1 text-left">
            ...
            <p className="text-[11px] leading-relaxed mt-1">
              Encontramos os detalhes do seu cadastro! VocÃª pode ajustar os campos abaixo ou preenchÃª-los manualmente caso algo esteja incorreto.
            </p>
          </div>
          ...
          <div className="space-y-4 text-left">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Ex: 12345-6 (NecessÃ¡ria para NFS-e)"
